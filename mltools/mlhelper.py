...
def missing_values(dataframe):
    ...
    missing_val_table =missing_val_table.rename(columns ={0:'Missing_Values',1:'Percent_of_Total_Values'})
    missing_val_table.index.names = ['Feature']
    missing_val_table.reset_index(level=0, inplace=True)
    sorted_missing  = missing_val_table[missing_val_table['Missing_Values']!=0].sort_values('Percent_of_Total_Values', ascending=False).round(2)
